...
//    }
    ...
    private void guiEmailKichHoat(String email, String maKichHoat) {
        ...
        emailService.sendEmail("cuongnguyen0834@gmail.com", email, subject, text);
    }
...
//    public ResponseEntity<?> DuLieuAo(String email, String maKichHoat) {
//        NguoiDung nguoiDung = nguoiDungRepository.findByEmail(email);
//        if (nguoiDung == null) {
//            return ResponseEntity.badRequest().body(new ThongBao("Người dùng không tồn tại"));
//        }
...
//    }
    ...
    public ResponseEntity<?> layThongTinNguoiDung(long maNguoiDung) {
    ...
    public ResponseEntity<?> capNhatThongTinNguoiDung(long maNguoiDung, NguoiDung nguoiDungCapNhat) {
        ...
        if (nguoiDungOptional.isPresent()) {
            ...
            // Cập nhật thông tin
            if (nguoiDungCapNhat.getDiaChiGiaoHang() != null) {
                nguoiDung.setDiaChiGiaoHang(nguoiDungCapNhat.getDiaChiGiaoHang());
            }
            if (nguoiDungCapNhat.getDiaChiMuaHang() != null) {
                nguoiDung.setDiaChiMuaHang(nguoiDungCapNhat.getDiaChiMuaHang());  // Fixed: using getDiaChiMuaHang instead of setDiaChiMuaHang
            }
            if (nguoiDungCapNhat.getEmail() != null) {
                nguoiDung.setEmail(nguoiDungCapNhat.getEmail());
            }
            if (nguoiDungCapNhat.getSoDienThoai() != null) {
                nguoiDung.setSoDienThoai(nguoiDungCapNhat.getSoDienThoai());
            }
            if (nguoiDungCapNhat.getTen() != null) {
                nguoiDung.setTen(nguoiDungCapNhat.getTen());
            }

            NguoiDung nguoiDungDaCapNhat = nguoiDungRepository.save(nguoiDung);
            return ResponseEntity.ok(nguoiDungDaCapNhat);
        }
